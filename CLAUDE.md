...
- Imágenes generadas (Higgsfield GPT Image 2.5, 0,5 créditos c/u): 6 productos de ejemplo, 2 de academia y 1 de estudio en `assets/img/gen/`. **Higgsfield quedó con 0,8 créditos** (un video cuesta ~8,75): el video de «El arte del detalle» (`assets/video/reel.mp4`) se hizo con ffmpeg a partir de 6 fotos de Claudia (acercamientos, paneos, transiciones) para no repetir el de la portada.
- Feedback de Jackson ya aplicado (23 sep, v5): letras más pequeñas y nunca recortadas (se comprueba con un detector de texto cortado en 4 tamaños y 2 idiomas: 0 problemas), sin marco dorado en la portada, videos distintos, portafolio en miniaturas con visor, agenda rediseñada, botones modernos (brillo + flecha en círculo), iconos reales de Instagram (degradado) y WhatsApp (verde, flotante en PC y en la barra del teléfono). Archivos CSS/JS con `?v=N` para que el teléfono no use copias viejas: subir N al cambiarlos.
- Jackson pidió conectar el MCP «motionsites» (`claude mcp add motionsites --scope user --transport http https://xgdzyqfalbibzelpdpvr.supabase.co/functions/v1/mcp`); el permiso de la app lo bloqueó: lo tiene que ejecutar él y abrir un chat nuevo.
