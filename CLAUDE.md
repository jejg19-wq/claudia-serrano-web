...
- `assets/js/gate.js` — **clave de acceso al sitio: `claudia`** (SHA-256 en el script, se recuerda por dispositivo). Para quitarla: borrar la etiqueta `<script src="…gate.js">` de cada página.
- `assets/fonts/` — TAN Mon Cheri reconstruida desde el subset del PDF con acentos/ñ sintetizados (`scratchpad/font_build.py` fue el generador). Italiana es respaldo para ¿ ? ! $.
...
3. **GitHub Pages: PENDIENTE.** `gh` no tiene sesión en esta máquina y Jackson no logró completar el código de dispositivo. Cuando pueda: `gh auth login` y luego `gh repo create claudia-serrano-web --public --source=. --push`, activar Pages en `main` / root. URL resultante `https://<usuario>.github.io/claudia-serrano-web/`.
4. **Higgsfield Websites (en curso como alternativa sin GitHub):** website creado con `create_website` → `website_id 03def297-059a-4191-8f3c-1d3d0db2c2a1`, slug `claudia-serrano`, tipo `website`, categoría `portrait-lifestyle`. El sitio empaquetado está subido como zip en Higgsfield: `https://d2ol7oe51mr4n9.cloudfront.net/user_3IjzGgnZuSTU9S9pksayizV4cm2/7581035f-cb77-4b78-9d54-87ba227324a2.zip` (106 archivos, sin banner/). Plan: `website_repo_access checkout` → en `sandbox_exec` descomprimir el zip dentro de `app/public/` para que se sirva como estático → commit → `website_repo_access push` → `deploy_website` → URL `https://claudia-serrano.<host>`. Si el flujo React estorba, servir `index.html` estático desde `public/` y dejar la ruta raíz redirigiendo a `/index.html`.
